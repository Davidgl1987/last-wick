...
- **Comportamiento base (rediseño 2026-08-31):** ya no patrulla — persigue al héroe sin descanso, pero deliberadamente más despacio que su paso WASD, así que el jugador puede abrirle hueco caminando. Dos umbrales de carga: uno **cercano** (como siempre) y uno nuevo **lejano** — al cruzar cualquiera de los dos se prepara (brilla y vibra ~0.8 s: el aviso) y luego **carga en línea recta** a gran velocidad hacia la última posición vista del héroe; en la banda intermedia entre ambos solo persigue, sin cargar. El umbral lejano existe para cerrar el hueco de quien se queda a distancia matándolo a proyectiles sin tocar nunca su carga.
  - **No carga si tiene una roca/muro demasiado cerca en la dirección de tiro** (playtest 2026-07-06): en ese caso no telegrafía; primero **retrocede alejándose del héroe** buscando línea despejada (rediseño 2026-08-31: antes iba al centro de la sala, lo que podía dejarlo congelado para siempre si el jugador se colocaba en diagonal) y solo carga cuando tiene recorrido. Evita el exploit de estrellarlo una y otra vez contra una roca pegada mientras el héroe le pega gratis al lado.
...
- **Debilidad:** casi inmune a cualquier daño mientras persigue o carga; solo es vulnerable de verdad durante el aturdimiento tras el choque. Ahí, la embestida del héroe (o unas flechas rápidas) hacen el daño grueso.
...
- **Arena:** sala alargada, dos pasillos laterales y un foco central donde vive la Reina.
- **Comportamiento base:** cada una de sus **columnas vivas** para minions por su cuenta, a un ritmo propio que se acelera por fase (simplificación 2026-08-31: sustituye la antigua oleada única sincronizada desde el cuerpo del jefe) — versiones débiles y de 1 solo golpe de un Dummy, sin peligro individual serio, que nacen del borde de la columna y **persiguen al héroe desde el primer momento** (playtest 2026-07-06: en línea recta no amenazaban; el reto llegaba tarde). No son la amenaza real por sí solas: son **ruido que ensucia la sala** si se dejan acumular, y sirven de escudo involuntario o de "moneda de cambio" para rebotes.
- **Columnas, su vida real (rediseño 2026-07-10):** el cuerpo de la Reina apenas tiene vida propia — está repartida en las **8 columnas** de su sala. Solo se rompen a embestida (los proyectiles rebotan sin efecto): **2 golpes** cada una, agrieta → rompe (bajado de 3 en la simplificación 2026-08-31, al retirarse la defensa de las larvas guardianas). Al romperse, el cuerpo pierde una porción de su vida máxima y la Reina queda **aturdida, vulnerable a daño completo,** unos segundos — la ventana real para hacerle daño grueso. Con las 8 rotas, queda vulnerable para siempre: el resto se remata a golpes normales.
- **La Reina te acecha:** persigue al héroe sin correa por toda la arena, rodeando columnas y rocas en vez de atravesarlas (rediseño 2026-07-10: antes derivaba con correa hacia su zona central), así que plantarse en un punto fijo a disparar deja de ser seguro — te va cerrando el espacio y acercando el rastro. Se enfurece con cada columna rota: acelera un poco por cada una (subida moderada a propósito desde el playtest 2026-08-31, que rebajó un primer tuning que llegaba a más que doblar la velocidad de paso del héroe con las 8 rotas). Sigue siendo control de espacio, no un boss de reflejos.
...
- **Larvas, cadencia por columna (simplificación 2026-08-31):** cada columna viva repone una larva cada **6 s en fase 1, 5 s en fase 2, 4 s en fase 3**, hasta un cupo total de **8 vivas** compartido por toda la sala — una columna rota deja de parir. Único rol desde esta misma pasada: perseguidora (se retiró la larva "guardiana" que orbitaba defendiendo su columna con embestida propia).
- **Fase 2 (66 %):** el rastro se genera más rápido y las columnas paren con más frecuencia; las larvas persiguen más rápido.
- **Fase 3 (33 %):** modo pánico — la Reina se mueve considerablemente más rápido (a estas alturas, con la mayoría de columnas ya rotas) y las que le queden paran a su cadencia máxima. Ganar significa limpiar hueco a tiros y arrinconarla.
- **Debilidad:** su ventana de vulnerabilidad no es un tiempo muerto propio como la de otros jefes — se dispara **al romperle una columna** (aturdida, daño completo unos segundos; con las 8 rotas, vulnerable para siempre). Fuera de esa ventana el daño sigue pasando, pero muy reducido. Tiene mucha vida y ningún ataque directo fuerte: el peligro es indirecto, quedarte sin sala limpia donde maniobrar. Premia jugar rápido y ordenado, castiga el tanteo lento.
