...
const DOOR_OPENING_WIDTH = 3.8;
const DOOR_Y_SCALE = 1.18;
const DOOR_Z = -3.15;
...
function HiddenEnemies() {
  return (
    <>
      <group position={[-3.05, 0.38, -1.65]} rotation-y={0.55} scale={0.66}>
      ...
      <group position={[3.25, 0.5, -2.05]} rotation-y={-0.45} scale={0.58}>
...
function Dust() {
  ...
  useFrame((state) => {
    ...
    const time = state.clock.elapsedTime;
    ref.current.rotation.y = Math.sin(time * 0.11) * 0.08;
    ref.current.position.y = Math.sin(time * 0.23) * 0.08;
  });
  return <points ref={ref} geometry={titleDustGeometry} material={titleDustMaterial} />;
}
...
function TitleCamera({ entering, onComplete }: { entering: boolean; onComplete: () => void }) {
  ...
  useFrame((state, delta) => {
    ...
    const idle = entering ? 0 : 1;
    camera.position.set(
      Math.sin(state.clock.elapsedTime * 0.18) * 0.055 * idle,
      THREE.MathUtils.lerp(2.75 + Math.sin(state.clock.elapsedTime * 0.14) * 0.025 * idle, 1.8, eased),
      THREE.MathUtils.lerp(8.2, -4.3, eased),
    );
    lookTarget.set(0, THREE.MathUtils.lerp(1.45, 1.75, eased), THREE.MathUtils.lerp(-1.2, -4.7, eased));
...
function Vestibule({ entering, onComplete }: { entering: boolean; onComplete: () => void }) {
  return (
    <>
      ...
      <hemisphereLight color="#7185b3" groundColor="#17131a" intensity={0.42} />
      <directionalLight color="#9caed8" intensity={0.7} position={[4, 7, 5]} />
      <WoodenFloor />
      <MansionDoor entering={entering} />
      <GroundedKitProp model="pillar" position={[-3.15, 0, -3.05]} scale={1.12} />
      <GroundedKitProp model="pillar" position={[3.15, 0, -3.05]} scale={1.12} />
      <GroundedKitProp model="wall_inset_shelves" position={[-4.25, 0, -2.8]} scale={0.95} />
      <GroundedKitProp model="wall_inset_candles" position={[4.25, 0, -2.8]} scale={0.95} />
      <GroundedKitProp model="banner_brown" position={[-2.55, 0.4, -2.83]} scale={0.78} warm />
      <GroundedKitProp model="banner_brown" position={[2.55, 0.4, -2.83]} scale={0.78} warm />
      <GroundedKitProp model="bench" position={[-3.35, 0, 0.35]} rotationY={Math.PI / 2} scale={0.78} warm />
      <HiddenEnemies />
      <Lumora />
      <Dust />
...
export function TitleScreenScene({ entering, onComplete }: { entering: boolean; onComplete: () => void }) {
  return (
    <Canvas
      ...
      camera={{ fov: 42, near: 0.1, far: 40, position: [0, 2.75, 8.2] }}
    >
